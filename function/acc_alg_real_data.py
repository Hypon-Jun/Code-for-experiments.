...
def beta_update_B(gamma, rho, mu, grad_function, prox_function, lamb):
    step_size = rho + mu
    grad = grad_function(gamma)
    B_tilde = gamma - grad / step_size
    weights_updated = prox_function(B_tilde[:-1], lamb, step_size)
    intercept_updated = B_tilde[-1].reshape(1, -1)
    B_new = np.vstack((weights_updated, intercept_updated))
    return B_new
...
def line_search_mu(X, Y, optimal_beta_B, optimal_beta_C, theta, mu, rho, L, mu_try,
    ...
    for search_iter in range(search_iterations):
        ...
        beta_B_try = beta_update_B(gamma_B_try, rho_try, mu_try, grad_B, prox_B_scad, lamb_B)
...
#主函数
def acc_robust_low_rank_learning_real_data(X, Y, lambda_B_avg, lambda_C_avg,B_init, C_init, max_iter=100000, tol=1e-6):
    #loss = create_objective_function(X, Y)
    n, p = X.shape
    _, m = Y.shape
    #lb_sum, lc_sum = lambda_B_avg * n, lambda_C_avg * n
    lb_sum, lc_sum = lambda_B_avg, lambda_C_avg
    mu = 0
    ...
    for iter in range(max_iter):
        ...
        B_beta = beta_update_B(B_gamma, rho, mu, gradient_B, prox_B_scad, lb_sum)
...
def acc_robust_low_rank_learning_with_line_search_real_data(X, Y, lambda_B, lambda_C, max_iter=100000, tol=1e-6):
    loss = create_objective_function(X, Y)
    n, p = X.shape
    _, m = Y.shape
    ...
    for iter in range(max_iter):
        ...
        if iter > 51:
            mu = line_search(X, Y, optimal_beta_B, optimal_beta_C, theta, mu, rho, L,
                    B_beta, C_beta, B_beta_tmp, C_beta_tmp, loss, lb_sum, lc_sum)
            rho = L - mu
            #print('mu',mu)
        if iter < 50:
            mu = mu0
